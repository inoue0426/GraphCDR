...
def dataload(
    ...
    mutation_feature = pd.read_csv(
        Genomic_mutation_file, sep=",", header=0, index_col=[0]
    )
    ...
    methylation_feature = pd.read_csv(
        Methylation_file, sep=",", header=0, index_col=[0]
    )
    assert (
        methylation_feature.shape[0]
        == gexpr_feature.shape[0]
        == mutation_feature.shape[0]
    )
    ...
    for each_drug in experiment_data_filtered.index:
        for each_cellline in experiment_data_filtered.columns:
            ...
            if (
                ...
                and each_cellline in mutation_feature.index
            ):
                if (
    ...
    print(
        "All %d pairs across %d cell lines and %d drugs."
        % (len(data_new), nb_celllines, nb_drugs)
    )
